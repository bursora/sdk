...
const MODEL = "gpt-4o";
const TAGS = { tenant_id: "playground-tenant", agent_id: "playground-agent", workflow_id: "playground-workflow" };
...
function loadEnv(): Env {
  ...
  return { endpoint, apiKey, cronSecret: process.env.BURSORA_CRON_SECRET || undefined, workspaceId: parts[1] };
}
...
interface Stats { ok: number; blocked: number; error: number }
...
function printOptions(label: string, opts: Options, env: Env): void {
  ...
  console.log(`  prompt=${opts.range.promptMin}-${opts.range.promptMax}  completion=${opts.range.completionMin}-${opts.range.completionMax}`);
...
async function anomaly(env: Env): Promise<void> {
  ...
  if (!env.cronSecret) {
    ...
    console.log(`    curl -H 'Authorization: Bearer <secret>' ${env.endpoint}/api/cron/anomaly`);
    return;
  }
