...
const coreFrom = (decisionClient: DecisionLookup, eventsClient: EventsClient): BursoraCore => ({
    decision: decisionClient,
    events: eventsClient,
    now: () => 1_000,
    flush: async () => {},
...
describe("wrap(openai)", () => {
    ...
    test("exposes a top-level flush() that delegates to the events client", async () => {
        const calls: { method: string; args: unknown }[] = [];
        const h = buildHarness();
        let flushed = 0;
        const client = makeClient(calls);
        const wrapped = wrap(
            client,
            coreFrom(h.decisionClient, {
                record: (e: RecordedEvent) => h.events.push(e),
                flush: async () => {
                    flushed += 1;
                },
            }),
    ...
    test("flush() does not throw even when the events transport rejects", async () => {
        const calls: { method: string; args: unknown }[] = [];
        const h = buildHarness();
        const client = makeClient(calls);
        const wrapped = wrap(
    ...
    test("does not shadow an existing flush() on the underlying client", async () => {
        const calls: { method: string; args: unknown }[] = [];
        const h = buildHarness();
        let underlyingFlush = 0;
        let bursoraFlush = 0;
        const client = {
            ...makeClient(calls),
            flush: async () => {
        ...
        await (wrapped as unknown as { flush: () => Promise<void> }).flush();
        expect(underlyingFlush).toBe(1);
        expect(bursoraFlush).toBe(0);
    });

    test("exposes a top-level dispose() that delegates to the events client", () => {
        const calls: { method: string; args: unknown }[] = [];
        const h = buildHarness();
        let disposed = 0;
        const client = makeClient(calls);
    ...
    test("does not shadow an existing dispose() on the underlying client", () => {
        const calls: { method: string; args: unknown }[] = [];
        const h = buildHarness();
        let underlyingDispose = 0;
        let bursoraDispose = 0;
        const client = {
            ...makeClient(calls),
            dispose: () => {
                underlyingDispose += 1;
